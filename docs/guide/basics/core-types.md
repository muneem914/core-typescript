...
```
***Analogy:** A string is like a label on a jar. You can read and change the text, but it’s always a series of characters. For example, a jar labeled "Sugar" can be relabeled as "Salt."*
### Number
...
```
***Analogy:** A number is like the speedometer in a car. It shows a numerical value, and you can’t accidentally replace it with words.*
### Boolean 
...
```
***Analogy:** A boolean is like a door — it’s either open (true) or closed (false). There’s no in-between.*
...
```
***Analogy:** An array is like an fruit carton that holds multiple fruits — they’re all the same type (fruits), just like elements in an array are the same type. No vegetables are allowed in the fruit cartoon.*
### Tuples
...
```
***Analogy:** A tuple is like a boarding pass. It has a fixed set of fields like passenger name and seat number. You can’t accidentally swap the name with the seat number.*
### Objects
...
```
***Analogy:** An object is like a toolbox — each tool (key) has a specific purpose (value).*
...
Enums can make easier to intent a document, or create a set of distinct cases. TypeScript provides both numeric and string-based enums.
### Numeric enums
...
***Analogy:** An enum is like traffic signals. You only have a few fixed options (red, yellow, green). You can’t accidentally invent a new signal.*
...
```
***Analogy:** `any` is like a backpack. You can put anything inside, but it might get messy if you’re not careful.*
...
```
***Analogy:** `unknown` is like a sealed package. You need to open it carefully to see what’s inside before using it.*
...
```
***Analogy:** `void` is like shouting into a canyon. You send a message (call the function), but you don’t expect anything back.*
...
```
***Analogy:** `never` is like a trapdoor. Once you step on it (call the function), you’re gone, and nothing comes back.*
